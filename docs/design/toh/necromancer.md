...
on a 60s cooldown, ignoring caps, and those expire after 20s. Ranks on
summon skills buy **summon stats**, not caster damage — stated per skill.
...
topping up as stacks expire. Both classify as `plague`.
...
and a slightly smaller damage number. **Not exclusive** — a player may take
both arms and pays two of four passive slots to be a generalist.

**7. Auras are not yet a primitive.** Osteo Aura and Blight are always-on
caster-following fields. They are marked `NEEDS aura` rather than forced
into `nova`, which is a one-shot circle. Both occupy a slot.
...
COST:                 fourteen damage on a ten-second cooldown is the worst
                      damage-per-cooldown ratio in the class. The entire
                      value is the root, and it pins one enemy out of a
                      swarm — a slot spent stopping a single thing while
                      everything else keeps walking
...
> This is invention, not derivation. **Converted to ranged on Casey's
> ruling** — was melee 84px on breach, now a 360px projectile.
...
> **Imperfect fit flagged:** a dash that damages everything along its path
> is not cleanly any existing primitive. `line` is the closest — everything
> in a straight path, uncapped — but `line` resolves instantly from the
> caster's position and this one carries the caster along it. If the engine
> can't move the caster as part of a `line`, this needs either a new
> primitive or the displacement expressed as a separate rider.
...
> **Heal-path flagged:** this is the class's only heal or drain, and the
> Necromancer is currently the only class of fourteen not touching the
> `compose.js` `p.hp` bypass (452/483/609). Building this makes it the
> fourteenth. Argument for fixing the bypass before implementation, not
> after.
...
> **Tier gap flagged:** the `Dark Matter` summon that sat at tier_code 4 is
> cut per ruling — it duplicated the tier-0 opener at 2.5× the Essence cost
> and its own COST line admitted it was "clearly better than neither." The
> tree is now ten nodes with tier_code 4 empty and 5 doubled. Left as-is
> rather than renumbered, since tree structure is deferred until shapes can
> be seen in-game.
...
> **`attackCd` unit warning:** the Monster's 1100ms swing must be declared
> in milliseconds. Four built summons across `hun_pincer` and `wd_swarm`
> declare seconds in this field and are the confirmed cause of the two worst
> `tree_dps` outliers (+4123% and +2313%). Necromancer's summons are
> currently clean. Do not introduce a fifth.
...
AXIS POSITION:        6 (of 10) — BRANCH PAIR 'necro_skeleton_branch'
                      (not exclusive — both arms may be taken)
--- delivery ---
CAST:                 n/a
SHAPE:                n/a
RANGE:                n/a
...
> Blood Skeleton is usually correct. Exclusivity is dropped roster-wide, so
> a player may take both and pay two of four passive slots for it.
...
PRIMITIVE:            NEEDS channel

> **Cooldown contradiction flagged, unresolved:** `PACE` says 9000ms while
> the block's prose has historically named a 3s cooldown measured from the
> channel's end. That claim is not expressible today — with no `channel`
> primitive there is nothing for "channel end" to be measured from. The
> block can be made consistent but not true until the primitive exists.
...
> **No spread radius.** `plague` seeds everything within `spreadRadius` once
> at cast; declaring no radius makes it a plain single-target DoT, which is
> what this skill is. The spreading here comes from `TARGET_UNAFFECTED`
> re-targeting, not from the primitive.
...
> **Rank-gated shape flagged, needs a ruling:** the rank-8 splash would make
> this `bolt + mortar` at high rank and plain `bolt` below it. Nothing else
> in the roster changes primitive with rank, and it is not known whether the
> engine can express that. Classified as `bolt` for now. If rank-gated
> primitives are impossible, either the splash becomes a rider on the bolt
> or the skill is `bolt + mortar` from rank 1 with the splash radius growing.
...
> flagged as such. The vulnerability is an impact rider (`defenseDown` is
> the built stand-in), not a primitive — the delivery is a projectile.
...
> **Matched to Entropy Cascade.** These are the same skill with different
> numbers; both are `plague` with no spread radius, spreading via
> `TARGET_UNAFFECTED` rather than via the primitive.
...
PRIMITIVE:            hazard + NEEDS gravity_pull

> **Not a trap.** A trap arms and waits to be stepped on; this is a placed
> persistent field that pulses eight times on its own. `hazard` is the
> correct delivery. The pull is the missing primitive — gravity pull is
> wanted by 11 nodes across 6 classes and exists today only as Mage trait
> machinery no skill can reach, so this node cannot ship until it is built.

---

## OPEN ITEMS

- **`NEEDS aura`** — Osteo Aura, Blight. 3 nodes across 2 classes roster-wide.
- **`NEEDS channel`** — Death Channel, Dark Energy Beam. 4 nodes, 3 classes.
- **`NEEDS gravity_pull`** — Singularity. 11 nodes, 6 classes; highest
  cross-class demand of any missing primitive.
- **Rank-gated primitive** — Dark Matter Bomb's rank-8 splash. May be unique
  in the roster; needs a ruling on whether the engine can express it.
- **Wrecking Ball** — `line` with caster displacement. May need a new
  primitive or the dash expressed separately.
- **`_heal` bypass** — Grasp of Death makes the Necromancer the fourteenth
  class on it. Fix before implementation.
- **Summons tier gap** — tier_code 4 empty, 5 doubled, after the Dark Matter
  cut. Deferred with all other tree-structure work.
