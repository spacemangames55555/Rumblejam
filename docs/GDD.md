...
| **11** | **Players deal no damage in the co-op run.** `d0:0, d1:0, hurt:true` every run; fights never clear, so three differently-named downstream timeouts are really one defect | **Open — outranks #8 and #9.** Those are intermittent lobby failures; this is the run itself being unplayable, and it reproduces every time |
