...
**The spec constrains structure, not node types.** The authored trees put a passive at tier 2 and one per branch at tier 6 because that is where their engine `scaleWeight` passives went — it is a pattern, not a rule. Chains carry between zero and two passives, and converting them does not add or move any. A converted branch may be four actives; `samurai_armor`'s absorb half carries the tree's one passive and its counter half carries none.
