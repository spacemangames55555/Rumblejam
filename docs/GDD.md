...
| **11** | **Only 2 of 47 characters can deal aimed damage.** Weapons are removed, so skills are the only damage source; `TREES_BY_CLASS` has two entries, and **not one of the 33 `classic` characters the game boots into has a tree.** Fights cannot be won, so they never clear, so everything downstream times out under a different name | **Diagnosed, not fixed — the remainder is phase-5 content.** The engine is fine: where a tree exists the character learns, auto-slots and kills (`toh_samurai` 162 dmg / 4 kills). `tools/offence_test.mjs` is the gate and fails until the trees exist. `sim_test` missed it because `nuke()` kills the enemies on the player's behalf |
