...
export const ENEMIES = [
  { id: 'skulker',   name: 'Skulker',   behavior: 'chaser',   hp: 8,  spd: 125, dmg: 4,  radius: 14, mats: 1,
    shape: 'circle',   color: '#e2504c', w: 3 },                                    // basic chaser
  { id: 'flit',      name: 'Flit',      behavior: 'sprinter', hp: 4,  spd: 215, dmg: 3,  radius: 11, mats: 1,
    shape: 'triangle', color: '#ffab4f', w: 2.2 },                                  // fast, fragile
  { id: 'slabjaw',   name: 'Slabjaw',   behavior: 'brute',    hp: 30, spd: 68,  dmg: 10, radius: 24, mats: 3,
    shape: 'square',   color: '#b03a3a', w: 1.4 },                                  // slow tank
  { id: 'lobber',    name: 'Lobber',    behavior: 'spitter',  hp: 9,  spd: 92,  dmg: 3,  radius: 14, mats: 2,
  ...
  { id: 'aegimand',  name: 'Aegimand',  behavior: 'warden',   hp: 22, spd: 78,  dmg: 5,  radius: 18, mats: 3,
    shape: 'hex',      color: '#5ea8ff', w: 1.2, shieldR: 150, shieldReduce: 0.5 }, // shields nearby allies
  { id: 'stitcher',  name: 'Stitcher',  behavior: 'medic',    hp: 11, spd: 118, dmg: 3,  radius: 13, mats: 3,
    shape: 'cross',    color: '#8be8c8', w: 1.2, healPs: 6, healR: 170 },           // heals allies, flees
  { id: 'wombden',   name: 'Wombden',   behavior: 'nest',     hp: 26, spd: 0,   dmg: 4,  radius: 22, mats: 3,
    shape: 'star',     color: '#c98b4f', w: 1.0, spawnCd: 4, maxBrood: 3, broodId: 'flit' }, // spawner
  { id: 'lancerfish', name: 'Lancerfish', behavior: 'dasher', hp: 11, spd: 105, dmg: 7,  radius: 14, mats: 2,
    shape: 'arrow',    color: '#4fd8eb', w: 1.6,
    dash: { windup: 0.55, speed: 560, dur: 0.45, cd: 2.4 } },                       // telegraphed dash
  { id: 'deadeye',   name: 'Deadeye',   behavior: 'sniper',   hp: 8,  spd: 62,  dmg: 6, radius: 13, mats: 2,
...
export const ELITE_MODS = [
  // 2% max hp / s while left alone, throttled to a TENTH of that for regenLockS
  // seconds after every hit.
  //
  // The throttle exists because the rate is a percentage of the entity's OWN
  // max HP while player damage is a flat number. On anything carrying a large
  // HP multiplier — a Bounty Hunt mark is 10x a boss fraction — the raw rate
  // outruns what a small party can land and the target stops being tough and
  // starts being unkillable. Measured on a solo mark: 135-151 HP/s of healing
  // against 105-110 HP/s landed.
  //
  // A FULL lock was tried first and over-corrected: against anything that fires
  // continuously the mod healed a measured 0.0 HP/s and became indistinguishable
  // from no modifier at all. A tenth keeps it a real cost that stays well under
  // any plausible damage output, and it does not depend on whether the attacker
  // happens to have gaps in their fire.
  { id: 'regenerating', name: 'Regenerating', regenPct: 0.02, regenLockS: 2, regenLockMult: 0.1 },
